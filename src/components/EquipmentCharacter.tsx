...
import { RaceAndClassSelector } from './RaceAndClassSelector';
import { CLASSES } from '../constants/classes';
import { RACES } from '../constants/races';
import { STAT_NAMES, BASE_STATS } from '../constants/stats';
import { TabPanel } from './TabPanel';

interface EquipmentCharacterProps {
  character: Character;
...
export const EquipmentCharacter = ({
  character,
  onCharacterChange,
  readonly = false,
}: EquipmentCharacterProps) => {
  ...
  const handleBonusChange = (slot: EquipmentSlot, stat: keyof CharacterStats, value: number) => {
    // Проверяем, что значение не станет отрицательным
    if (value < 0) return;
